...
class Order:
    ...
    def cancel(self):
        if self.status == 'progress':    
            kitchen_base_url = "http://127.0.0.1:3658/m1/1072221-1060503-default/kitchen"
            ...
            if response.status_code == 200:
                ...
                raise APIIntegrationError(f"Failed to cancel order with id {self.id}: {response.text}")
    ...
    def pay(self):
        response = requests.post(      
            'http://127.0.0.1:3658/m1/1072866-1061173-default/payments', json={'order_id': self.id}
        )
    ...
    def schedule(self):
        response = requests.post(         
            'http://127.0.0.1:3658/m1/1072221-1060503-default/kitchen/schedules',
            json={'order': [item.dict() for item in self.items]}
        )
        if response.status_code == 201:     
